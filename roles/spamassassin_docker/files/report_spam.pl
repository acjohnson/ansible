...
if(
   ...
   foreach my $file( @files ){
      if( -r $path . $file ){
         ...
         system( "docker exec -it spamassassin $spamassassin -r < $path$file" );
         system( "docker exec -it spamassassin $sa_learn --spam --no-sync $path" );
      } else {
...
system( "docker exec -it spamassassin $sa_learn --sync" );
